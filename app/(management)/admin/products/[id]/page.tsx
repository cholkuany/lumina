    ...
    <div className="space-y-8">
      ...
      <div className="flex items-center justify-between">
        <nav className="flex items-center gap-2 text-sm">
          <Link
            href="/admin/products"
            className="text-border-dark hover:text-primary transition-colors flex items-center gap-1"
          >
        ...
        <Link
          ...
          className="flex items-center gap-2 px-4 py-2 bg-text-primary text-white text-sm 
          font-medium rounded-xl hover:bg-text-primary/85 transition-colors"
        >
      ...
      <div className="flex items-center gap-2 flex-wrap">
        {product.isNewArrival && (
          <span className="px-3 py-1 bg-text-primary text-white text-xs font-medium rounded-full">
            New Arrival
          </span>
        )}
        {product.isSale && (
        ...
        {product.isFeatured && (
          <span className="px-3 py-1 bg-primary/15 text-primary-dark text-xs font-medium rounded-full">
            Featured
          </span>
        )}
        {totalStock === 0 ? (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
              ...
              <div className="space-y-3">
                <div className="relative aspect-square rounded-xl overflow-hidden bg-surface">
                  ...
                  {discountPercentage > 0 && (
                    <span className="absolute top-3 right-3 px-2 py-1 bg-primary text-white text-xs font-semibold rounded-lg">
                      -{discountPercentage}%
                    </span>
                  )}
                </div>
              ...
              <div className="space-y-4">
                <div className='space-y-1'>
                  {product.brand && (
                    <p className="text-sm text-border-dark font-medium uppercase tracking-wider">
                      {product.brand}
                    </p>
                  )}
                ...
                <div className="flex items-baseline gap-3">
                  ...
                  {product.variants[0].originalPrice && (
                    <span className="text-lg text-border-dark line-through">
                ...
                {/* Category */}
                <div className="space-y-1">
                  <p className="text-xs text-border-dark uppercase tracking-wider font-medium">
                    Category
                  </p>
                  <div className="flex items-center gap-1.5 flex-wrap text-sm">
                    {product.category.ancestors?.map((ancestor, idx) => (
                      <span key={idx} className="flex items-center gap-1.5">
                        <span className="text-border-dark">{ancestor}</span>
          ...
          <div className="bg-white rounded-xl border border-border p-6 space-y-4">
            <div className="flex items-center gap-2">
              ...
              <h2 className="text-lg font-medium text-text-primary">
                Variants
                <span className="ml-2 text-sm font-normal text-border-dark">
              ...
              {product.variants.map((variant, idx) => (
                ...
                >
                  ...
                  <div className="flex-1 grid grid-cols-2 sm:grid-cols-4 gap-3">
                    <div>
                      <p className="text-xs text-border-dark">SKU</p>
                      <p className="text-sm font-medium text-text-primary truncate">
                        {variant.sku}
                      </p>
                    </div>
                    <div>
                      <p className="text-xs text-border-dark">Attributes</p>
                    ...
                    <div>
                      <p className="text-xs text-border-dark">Price</p>
                      <div className="flex items-center gap-1.5">
                        <p className="text-sm font-medium text-primary">
                          ${variant.price.toFixed(2)}
                        </p>
                        {variant.originalPrice && (
                          <p className="text-xs text-border-dark line-through">
                    ...
                    <div>
                      <p className="text-xs text-border-dark">Stock</p>
                      <p
              ...
              <div className="divide-y divide-border">
                {product.specifications.map((spec, idx) => (
                  <div key={idx} className="flex items-center py-3 gap-4">
                    <p className="text-sm text-border-dark w-40 shrink-0 capitalize">
                      {spec.key}
                    </p>
        ...
        <div className="space-y-6">
          ...
          {/* Stats */}
          <div className="bg-white rounded-xl border border-border p-6 space-y-4">
            <h2 className="text-lg font-medium text-text-primary">Stats</h2>
            <div className="space-y-3 divide-y divide-border">
              ...
              <div className="flex items-center justify-between py-3">
                ...
                <span className={`text-sm font-semibold ${totalStock === 0 ? 'text-red-500' : 'text-text-primary'
                  }`}>
                  {totalStock} units
                </span>
              </div>
                  ...
                  return (
                    <div key={star} className="flex items-center gap-3">
                      <div className="flex items-center gap-1 w-12 shrink-0">
                        <span className="text-xs text-border-dark">{star}</span>
                      ...
                      <span className="text-xs text-border-dark w-8 text-right">
                        {percentage}%
                      </span>
                    </div>
                  )
          ...
          <div className="bg-white rounded-xl border border-border p-6 space-y-4">
            <h2 className="text-lg font-medium text-text-primary">Stock Summary</h2>
            <div className="space-y-2">
              {product.variants.map((variant, idx) => {
                ...
                return (
                  <div key={idx} className="space-y-1">
                    <div className="flex items-center justify-between">
                      <span className="text-xs text-border-dark truncate max-w-32">
                        {variant.attributes.color} / {variant.attributes.size}
                      </span>
